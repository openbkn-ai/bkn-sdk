...
export interface TraceContextOptions {
  ...
  traceparent?: string;
  /**
   * Business conversation id, supplied by the caller. Only the caller knows
   * where a conversation starts and ends, so the SDK never generates one:
   * omit it and the request stays uncorrelated. An issuer prefix is allowed
   * (e.g. `agent:thread_x`).
   */
  conversationId?: string;
  /**
   * Interaction id for one user question and every call it triggers, supplied
   * by the caller. Never generated by the SDK — a per-client id would collapse
   * unrelated rounds into one false interaction.
   */
