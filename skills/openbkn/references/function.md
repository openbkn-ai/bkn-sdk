...
| Command | Notes |
|---------|-------|
| `sandbox template [--type python]` | 入口骨架（`--type` 目前只收 `python`）。**入口函数必须叫 `handler`**，签名 `handler(event) -> Any`。 |
| `sandbox deps` / `sandbox versions <package>` | 查看沙箱已安装库，或实时查询包索引。 |
| `sandbox run <file\|-> [--event '<json>'] [--timeout <s>] [--dep name@version] [--index-url <url>] [--pass-token]` | 跑一次代码。`--event` 是 `handler` 入参；`-` 从 stdin 读代码；`--timeout` 必须是正整数（秒）。 |
| `sandbox infer-schema <file>` | 从 `@tool` 装饰的代码反推参数定义；会真的执行代码。 |
| `sandbox generate <type> [--timeout <s>]` / `sandbox prompt <type>` | 让平台模型生成函数代码或参数元数据（客户端默认等 300 秒；默认网关 60 秒即 504），或读取对应提示词模板。 |
...
**超时有两道墙**：`--timeout` 抬的是沙箱与客户端的预算，但网关（ingress-nginx）自己有
读超时，超过就是 504，跟 `--timeout` 填多大无关。foundry 的 chart 没有调高它，默认 **60 秒**
（14.103.77.23 实测：75 秒的 `sandbox run` 在 60 秒收到 504；记在 openbkn-ai/bkn-foundry#1641 的评论里，该 issue 本身跟踪的是下面工具箱代理约 30 秒的切断）；
部署方调高了 `proxy-read-timeout` 才能更长。长任务别指望同步等。
`sandbox generate` 是一次模型调用，客户端默认等 300 秒（不是通用的 30 秒），`--timeout <s>` 可调；
但在默认网关下，超过 60 秒的生成同样会 504。
