...
| `eval-set test <cases.json> --agent <id> [--llm]` | Run each case through the agent, fetch the trace, check assertions (contains/regex/tool_call_*/latency deterministic; `semantic_match` needs `--llm`). |
