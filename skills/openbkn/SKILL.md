...
| `sandbox` | 临时代码（不注册任何东西） | `run <file> --event '<json>' [--timeout <s>] [--pass-token]`、`infer-schema`、`deps`、`versions`、`template`、`generate <type>`/`prompt <type>`（平台模型生成代码/参数元数据）。入口函数必须叫 `handler`；成败看 `exit_code` 不是 HTTP 码；长任务留在这里跑 |
