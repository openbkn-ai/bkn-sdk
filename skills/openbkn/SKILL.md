...
| `context` | MCP 检索 | `search-schema`、`query-object-instance`、`find-skills`、`tools`、`tool-call <name> [--arg k=v]`、`call-method <method>`、`resources/templates/prompts`、`query-instance-subgraph`/`get-logic-properties`/`get-action-info`；新工具用 `tools` 发现 + `tool-call` 调用，无需改 CLI |
