...
| Concern | Choice | Why |
| ------- | ------ | --- |
| Language | TypeScript (ESM), Node ≥ 22 | Both legacy repos are TS; ≥22 (kweaver-sdk's floor) for stable native fetch / test runner |
| CLI framework | **commander** | Mature, 0 runtime deps, clean command tree, biggest ecosystem; equivalence-friendly (legacy `kweaver-admin` uses it). Needs a custom grouped-help renderer (see below) |
...
| Validation | **zod** | Parse at IO boundaries (argv, HTTP responses); already used by kweaver-sdk |
| HTTP | native `fetch` | No axios/node-fetch dep; Node ≥22 has it |
| Build | **tsup** | Dual output (library + `openbkn` bin) in one config; from kweaver-admin. Replaces kweaver-sdk's hand-rolled `tsc` + `cp` script |
| Test | **vitest** | One runner, UT + coverage; from kweaver-admin. Replaces kweaver-sdk's `node:test` + tsx |
...
The legacy `kweaver` help is **not** commander's default flat list — it groups
...
- Render section headers in the legacy order, plus a new `OPERATOR` group for the merged `kweaver-admin` commands (org/user/role/audit).
...
This is one small shared formatter module, not per-command help strings. Equivalence is enforced full-depth (154 kweaver + 43 admin paths) by the equivalence test.
...
| Tool | kweaver-sdk | kweaver-admin |
| ---- | ----------- | ------------- |
