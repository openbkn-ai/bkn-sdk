...
| Unit tests | green, no external deps | ✅ `npm test` = 221 passed / 1 skipped. Per-domain mocked-fetch UT across all real domains + pure-logic suites (PK detection, bkn validate, tar/zip round-trips, SSE stream parse, trace predicates, eval-set assertions, schema validate). Live LLM-judge test gated behind `BKN_JUDGE_LIVE=1` |
