...
| Auth OAuth flows | auth | `auth login` supports `--token` attach; browser OAuth2 + password signin (+ legacy flags --alias/--client-id/--refresh-token/--port…) staged in `auth/oauth.ts` pending verified backend contract. | High |
