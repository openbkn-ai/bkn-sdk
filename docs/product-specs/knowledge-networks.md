...
- `relation-type-paths` passes the body `direction` through unchanged. The backend accepts `forward | backward | bidirectional` and refuses `reverse` with 400 (verified live), although the contract's enum lists `reverse`. It keeps sending `X-HTTP-Method-Override: GET`, which the backend requires though the spec does not declare it.
