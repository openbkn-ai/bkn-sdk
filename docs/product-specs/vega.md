...
- `openbkn vega resource document-*` — create and upsert one dataset document at a time; reads and deletes accept one or more document IDs, with optional `--ignore-missing`.
