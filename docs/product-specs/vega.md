...
- `openbkn vega discover-schedule …` — create/list/get/update/delete discovery schedules and enable or disable them explicitly. Full updates require the current `catalog_id` and `enabled`; `--expected-update-time` enables optimistic locking.
...
- `vega.discoverSchedules`, `get/create/update/deleteDiscoverSchedule`, and the enable/disable actions cover the full DiscoverSchedule contract. Schedule updates require `catalogId`, `enabled`, `startTime`, `endTime`, and `expectedUpdateTime`, mapped to the backend's strict replacement fields.
...
- DiscoverSchedule PUT is a strict replacement. Callers must send the unchanged `catalogId`, current `enabled`, and both time-window bounds (`0` means unbounded); enable/disable transitions use the action methods.
