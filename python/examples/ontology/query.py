...
def main() -> None:
    ...
    key, *rest = properties
    print(f"{Order.__name__}: {Order.count()} rows, properties {properties[:6]}")
    ...
    if some:
        ...
        print(f"{key} == {sample!r}: {narrowed.count()} rows")

        # `~` rewrites to the opposite operator rather than wrapping in a `not`
        # the backend does not have.
        print(f"negated: {Order.where(~(getattr(Order, key) == sample)).count()} rows")
    ...
    if rest:
        ...
        print(f"page: {len(page.rows)} rows, total {page.total}")
    ...
    if first is not None and len(Order.__primary_key__) == 1:
        ...
        print(f"get(primary key): {one.__instance_id__ if one else 'no match'}")
    ...
    if hops and some:
        ...
        print(f"{hops[0]}: {len(hop.take(2))} rows (target {hop.object_type.__name__})")
