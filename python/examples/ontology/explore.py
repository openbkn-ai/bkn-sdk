...
"""From a question to rows, without knowing the network's names first.
    ...
    BKN_KN_ID=ecommerce_ops_bkn_public python examples/ontology/explore.py "recent large orders"
...
def main(question: str) -> None:
    ...
    print(f"object types this question touches: {types[:6]}")
    ...
    if recall is not None and not nodes:
        ...
        print(
            f"  no instances recalled: {recall.get('message', 'nothing indexed in this network')}"
        )
    ...
    if not types or not types[0]:
        ...
        print(f"no search result; using {types[0]}")
    ...
    try:
        print(f"\n{cls.__name__}: {cls.count()} rows")
    except HttpError as error:
        print(f"\n{cls.__name__} cannot be read: {error.body[:120]}")
        return
...
if __name__ == "__main__":
    main(sys.argv[1] if len(sys.argv) > 1 else "orders")
